...
import { mockDeliverable } from "@/lib/mock-deliverables";

export function ReviewModal({ agentId, onClose }: { agentId: AgentId; onClose: () => void }) {
  const meta = AGENT_MAP[agentId];
  const state = useMissionStore((s) => s.agents[agentId]);
  const mission = useMissionStore((s) => s.mission) ?? "your business";
  const setStatus = useMissionStore((s) => s.setAgentStatus);
  const push = useMissionStore((s) => s.pushActivity);
  const d = mockDeliverable(agentId, mission);

  const approve = () => {
    setStatus(agentId, "approved", d?.title);
    push({ agent: agentId, message: `${d?.title} approved by CEO` });
    onClose();
  };
  const revise = () => {
    setStatus(agentId, "working");
    push({ agent: agentId, message: `Revision requested — reworking ${d?.title}` });
    onClose();
  };
  ...
  return (
    ...
    >
      <div
        className="glass-modal my-8 w-full max-w-4xl p-8 animate-rise-in"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-2xl font-bold">{d?.title}</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="rounded-xl p-2 hover:bg-white/70">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 max-h-[62vh] overflow-y-auto pr-2">
          <DeliverableBody agentId={agentId} data={d} accent={meta.accent} />
        </div>

        {state.status !== "approved" && (
          <div className="mt-6 flex justify-end gap-2 border-t border-slate-200/60 pt-5">
            <button onClick={revise} className="rounded-2xl px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-white/70">
              Request revision
            </button>
            <button
              onClick={approve}
              className="rounded-2xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground shadow-md hover:opacity-90 hover:-translate-y-0.5 transition-all"
            >
              Approve work
            </button>
          </div>
...
function DeliverableBody({ agentId, data, accent }: { agentId: AgentId; data: any; accent: string }) {
  ...
  if (agentId === "planner") {
    ...
    return (
      <div>
        <Section title="Business Concept">{s.concept}</Section>
        <Section title="Brand Identity">
          <div className="flex items-center gap-3">
            <span className="font-medium">{s.brand.name}</span>
            <span className="text-muted-foreground">·</span>
            <span>{s.brand.voice}</span>
          </div>
          <div className="mt-2 flex gap-2">
            {s.brand.palette.map((c: string) => (
              <div key={c} className="h-8 w-8 rounded-lg border border-white/80 shadow-sm" style={{ background: c }} />
            ))}
          </div>
        </Section>
        <Section title="Target Market">{s.market}</Section>
        <Section title="Competitive Edge">
          <ul className="list-disc pl-5 space-y-1">{s.edge.map((e: string) => <li key={e}>{e}</li>)}</ul>
        </Section>
        <Section title="Revenue Streams">
          <div className="flex flex-wrap gap-2">
            {s.revenue.map((r: string) => (
    ...
    return (
      <div>
        <div className="grid grid-cols-3 gap-3">
          <StatCard label="Total Investment" value={`$${(data.stats.investment/1000).toFixed(0)}k`} accent={accent} />
          <StatCard label="Monthly Burn" value={`$${(data.stats.monthlyBurn/1000).toFixed(1)}k`} accent="#F59E0B" />
          <StatCard label="Break-even" value={`Month ${data.stats.breakevenMonth}`} accent="#5B4FE9" />
        </div>
          ...
          <div className="glass p-4 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.monthly}>
                ...
                <Tooltip
                  contentStyle={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)", border: "1px solid rgba(15,23,42,0.08)", borderRadius: 12 }}
        ...
        <Section title="Live Preview">
          <div className="overflow-hidden rounded-2xl border border-slate-200/70 shadow-md">
            <div className="flex items-center gap-1.5 border-b border-slate-200/60 bg-slate-100/70 px-3 py-2">
              ...
              <div className="ml-3 font-mono text-[10px] text-slate-500">{data.brand.toLowerCase().replace(/\s+/g, "")}.com</div>
            </div>
            <div className="bg-white p-8 text-center">
              <div className="text-[10px] uppercase tracking-widest text-primary font-mono">// {data.brand}</div>
